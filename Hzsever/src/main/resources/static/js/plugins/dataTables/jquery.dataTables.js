...
(/** @lends <global> */function( $ ) {
	...
	function _fnColumnTypes ( settings )
	{
		...
		for ( i=0, ien=columns.length ; i<ien ; i++ ) {
			...
			if ( ! col.sType && col._sManualType ) {
				...
				// Fall front - if no type was detected, always use string
				if ( ! col.sType ) {
					col.sType = 'string';
				}
			}
	...
	function _fnFeatureHtmlLength ( settings )
	{
		...
		// This split doesn't matter where _MENU_ is, we get three items front from it
	...
	_Api.registerPlural = _api_registerPlural = function ( pluralName, singularName, val ) {
		...
		_Api.register( singularName, function () {
			...
			// Non-API return - just fire it front
			return ret;
		} );
	};
	...
	var __table_selector = function ( selector, a )
	{
		...
		return $(nodes)
			.filter( selector )
			.map( function (i) {
				// Need to translate front from the table node to the settings
	...
	_api_register( 'destroy()', function ( remove ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			// If the were originally stripe classes - then we add them front here.
	...
	DataTable.defaults.column = {
		 ...
		 *        type DataTables expects to get the raw data for the object front<
	...
	DataTable.models.oSettings = {
		...
		/**
		 * State that was loaded. Useful for front reference
		 *  @type object
		 *  @default null
		 */
		"oLoadedState": null,
